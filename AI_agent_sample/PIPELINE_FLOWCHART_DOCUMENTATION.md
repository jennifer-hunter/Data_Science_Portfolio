...
> **System Status Note:**
> This documentation describes the complete architecture of a self-improving AI content pipeline. The system is **production-capable** with the following implementation status:
>
> **✅ Fully Operational:**
...
>
> **⚠️ Modified for GDPR Compliance:**
> - **Performance tracking loop:** Platform analytics collection is currently paused due to GDPR considerations (low/no engagement metrics may constitute identifying information). The continuous learning system operates as described using **manual training data** in place of automated social media metrics. All feedback loops remain functional with human-curated quality assessments.
>
> **🚧 Under Active Development:**
> - Music selection agent (beat-sync and mood matching)
> - Hashtag quality scoring and generation optimization
> - Video creation quality assessment refinement
>
> The architecture supports seamless integration of automated analytics when regulatory compliance is established.
...
10. [API & Tool Reference](#10-api--tool-reference)
...
graph TB
    ...
    Posted -->|Yes| Performance[Performance Tracking]
    Posted -->|No| ErrorLog[Error Logging]
...
### 4.1 Video Generation & Enhancement
...
graph TB
    ...
    StatusCheck -->|Failed| ErrorHandle2[Error Handler<br/>Classify error type]
    ...
    MoviePyEnhance -->|Yes| MoviePyAgent[MoviePy Reel Creator<br/>moviepy_reel_creator_agent.py]
    ...
    UpdateVidDB --> QualityGateVid

    ErrorHandle2 --> ClassifyError{Error Type?}
...
graph TB
    ...
    ContentCheck -->|Video| VideoJudge[Video Judge<br/>video_quality_judge.py]

    PhotoJudge --> LoadWeights1[Load Training Weights]
    LoadWeights1 --> WeightFiles1{Weight Files}
    WeightFiles1 --> PollyWeights[hashtag_weights.json<br/>Historical training data]
    ...
    PollyWeights --> GPTVision1[GPT-4o Vision Analysis]
    ...
    AnalyzeImage --> FashionStyle[Fashion/Style: 0-10<br/>Luxury aesthetic, trends]
    ...
    PredictEngagement --> SavePrediction[(Store Prediction<br/>for Learning Loop)]
    ...
    VideoScoring --> BrandAlignment[Brand Alignment: 0-10<br/>Premium brand aesthetic]
    ...
    SaveVideoReject --> ReturnToGen2[Return to Video Generation]

    SaveVideoPass --> SavePrediction
    SavePrediction --> NextStage([Continue to Text Generation])

    ReturnToGen1 -.->|Feedback Loop| QualityStart
    ReturnToGen2 -.->|Feedback Loop| QualityStart
```
...
### 6.1 Caption & Hashtag Generation Flow
...
graph TB
    ...
    VisualContext --> Scene[Scene: beach, city, interior]
    VisualContext --> Objects[Objects: fashion, props]
    VisualContext --> Colors[Colors: warm, cool, vibrant]
    VisualContext --> Mood[Mood: luxury, casual, energetic]
    VisualContext --> People[People: poses, expressions]
    ...
    SaveCaption --> HashtagGen[Hashtag Generator<br/>video_hashtag_generator.py]
    ...
    FinalPackage --> PostPackage{Post Package Ready}
    PostPackage --> FinalContent[Content: Image/Video file<br/>High-quality, approved]
    PostPackage --> FinalCaption[Caption: Engaging text<br/>Hook + Body + CTA]
    PostPackage --> FinalHashtags[Hashtags: 20-30 tags<br/>Strategic mix]

    FinalContent --> QueueStage([Continue to Queue Generation])
    FinalCaption --> QueueStage
    FinalHashtags --> QueueStage
```
...
### 7.1 Queue Generation & Publishing Flow
...
graph TB
    ...
    QueueDB --> PublisherMenu[Social Media Publisher<br/>social_media_publisher.py]
    ...
    PostSuccess -->|Yes| SaveToPublishedDB[(DB: published_posts<br/>Account-specific database<br/>Platform ID and timestamp and metadata)]
    PostSuccess -->|No| ErrorHandler
    ...
    NextInQueue -->|No| PublishComplete[Publishing Complete]
...
### 8.1 Performance Tracking & Continuous Learning
...
graph TB
    ...
    CollectionSummary --> LearningLoop[Continuous Learning Loop<br/>continuous_learning_loop.py]
    HashtagReport --> LearningLoop
    SaveThemePerf --> LearningLoop
    ...
    FeedbackLoop -.->|Improves Quality| PromptGeneration[Prompt Generation]
    FeedbackLoop -.->|Optimizes Selection| ThemeSelection[Theme Selection]
    FeedbackLoop -.->|Raises Standards| QualityScoring[Quality Scoring]
    FeedbackLoop -.->|Refines Tags| HashtagGeneration[Hashtag Generation]

    PromptGeneration -.->|Better Content| TrackStart
    ThemeSelection -.->|Better Content| TrackStart
    QualityScoring -.->|Better Content| TrackStart
    HashtagGeneration -.->|Better Content| TrackStart
```
...
### 9.1 Google Sheets Calendar Sync
...
graph TB
    ...
    ApplyFormatting --> SetPermissions[Set Sheet Permissions<br/>Team collaboration<br/>Comment, view, edit rights]
...
## 10. API & TOOL REFERENCE

### 10.1 Content Generation APIs
...
**API Keys:**
- `OPENAI_API_KEY` - GPT-4/4o/4o-Vision
- `IMAGE_API_KEY` - Image generation
- `VIDEO_API_TOKEN` - Video generation
- `PLATFORM_ACCESS_TOKEN` - Publishing + analytics (per account)
- `GOOGLE_CREDENTIALS_JSON` - Drive/Sheets/GCS (OAuth2)

**Current Implementation:**
- Images: Replicate FLUX.1 Krea [dev]
- Videos: Replicate Kling 2.1

**Python Stack:**
`openai` | `replicate` | `requests` | `google-api-python-client` | `google-auth` | `moviepy` | `librosa` | `Pillow` | `ffmpeg-python`

---

### 10.4 Rate Limits & Best Practices
...
### 11.1 Content Generation Databases
...
### 11.2 Video & Enhancement Databases
...
graph TB
    ...
    VideoDB --> Quality[video_quality_scores<br/>score_id PK<br/>video_id FK<br/>technical_quality<br/>engagement_potential<br/>brand_alignment<br/>motion_quality<br/>overall_score<br/>passes_quality_gate<br/>rejection_reasons JSON]
```

---

### 11.3 Publishing & Performance Databases
...
### 11.4 Learning & Training Databases
...
graph TB
    ...
    LearningDB --> Evals[evaluations<br/>evaluation_id PK<br/>content_id FK<br/>content_type<br/>scene_quality<br/>mood_alignment<br/>lighting_quality<br/>composition<br/>fashion_style<br/>overall_score<br/>visual_elements JSON<br/>strengths JSON<br/>weaknesses JSON<br/>predicted_engagement<br/>passes_quality_gate<br/>decision]
...
### 12.1 Error Classification (Overview)
...
graph TB
    ...
    ClassifyError -->|API Failure| APIError[API Error Handler<br/>See Section 12.2]
    ClassifyError -->|Publishing| PublishError[Platform Error Handler<br/>See Section 12.3]
    ClassifyError -->|Infrastructure| InfraError[Infrastructure Error Handler<br/>See Section 12.4]
    ClassifyError -->|Content Rejection| ContentError[Content Error Handler<br/>See Section 12.5]
...
### 12.3 Platform Publishing Errors
